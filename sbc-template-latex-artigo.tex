...
A navegação pelas telas do sistema deve ser feita de forma sequencial e orientada pelas necessidades do gestor hospitalar. Inicialmente, o usuário acessa o dashboard geral, onde encontra indicadores em tempo real de ocupação de leitos, tempo de espera e alertas críticos; essa visão serve como ponto de partida para identificar situações de superlotação iminente. Em seguida, pode avançar para a tela de gestão de pacientes, que organiza os atendimentos por nível de urgência e apresenta dados clínicos e sinais vitais, permitindo priorizar casos críticos.\\
\\
Depois, o usuário pode consultar a tela de gestão de leitos, que mostra a disponibilidade, bloqueios e higienizações, facilitando a alocação rápida de pacientes. A seção de alocação inteligente complementa esse processo ao sugerir, com base em inteligência artificial, a melhor distribuição de pacientes nos setores disponíveis. Para planejamento estratégico, a tela de previsão de demanda apresenta análises preditivas de ocupação e períodos de pico, ajudando na antecipação de recursos. Por fim, as áreas de relatórios e indicadores e de auditoria do sistema permitem avaliar desempenho, eficiência da IA e histórico de acessos, enquanto as configurações possibilitam ajustar integrações, dados institucionais e preferências de segurança. Dessa forma, a navegação segue um fluxo lógico: monitoramento → priorização → alocação → previsão → avaliação → ajustes.
...
{images/apresentação de telas.pdf}
